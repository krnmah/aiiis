...
class EmbeddingService:
    ...
    def _load_model(self) -> SentenceTransformer:
        ...
        if self._model is None:
            logger.info(
                "embedding_model_load_started", extra={"model_name": self._model_name}
            )
            self._model = SentenceTransformer(self._model_name)
            logger.info(
                "embedding_model_load_complete", extra={"model_name": self._model_name}
            )
        return self._model
    ...
    def embed_text(self, text: str) -> list[float]:
        ...
        logger.info(
            "embedding_generated", extra={"embedding_dimension": int(vector.shape[0])}
        )
